...
while True:
    ...
    if i_episode % save_ckpt_interval == 0 and os.path.isfile(os.path.join(ckpt_dir, "ckpt_eps%d.pt" % i_episode)):
        policy_net, target_net, optimizer, memory, i_episode, episode_durations, episode_loss = \
    ...
    for t in count():
        ...
        if done:
            ...
            mean_duration = (sum(episode_durations[-100:]) / 100) if len(episode_durations)>=100 else 0
            ...
            print("Episode: %d Cumulative Rewards: %d Episode Loss: %f, past 100 episodes avg reward: %f"
                  % (i_episode + 1, t + 1, (running_loss / (t + 1)), mean_duration))
    ...
    if (i_episode + 1) % save_ckpt_interval == 0:
        save_checkpoint(ckpt_dir, policy_net, target_net, optimizer, memory, i_episode + 1, episode_durations, episode_loss)
